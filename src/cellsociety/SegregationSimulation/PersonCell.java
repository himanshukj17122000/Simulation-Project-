...
public class PersonCell extends Cell {
    ...
    @Override
    public void updateCell(GridPane grid, GridEntry entry, Set<GridEntry> emptyCellSet) { //need to fix to get empty cell set somewhere or make new method
        Boolean satisfied = checkSatisfaction(entry);
        if(!satisfied){
            int space = new Random().nextInt(emptyCellSet.size());
            int i = 0;
            for(GridEntry gridSpace : emptyCellSet) {
                if (i == space){
                    Cell newEmptyCell = new cellsociety.PreySimulation.EmptyCell(entry);
